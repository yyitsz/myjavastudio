...
namespace Framework.Utils
{
    ...
    public class DefaultBinder : Binder
    {
        ...
        public DefaultBinder(Type[] genericTypes)
            : base()
        {
            ...
            )
        {
            ...
            for (int i = 0; i < match.Length; i++)
            {
                ...
                {
                    if (match[i].IsGenericMethod == false && this.genericTypes.Length == 0)
                    {
                        return match[i];
                    }
                    else if (match[i].IsGenericMethod && match[i].GetGenericArguments().Length == this.genericTypes.Length)
                    {
                        return ((MethodInfo)match[i]).MakeGenericMethod(this.genericTypes);
            ...
            )
        {
            ...
            if ((valueType.IsInterface || valueType.IsClass)
                && (myChangeType.IsInterface || myChangeType.IsClass)
                && myChangeType.IsAssignableFrom(valueType))
            {
                return value;
            }
            ...
            for (int i = 0; i < match.Length; i++)
            {
                ...
                for (int j = 0; j < types.Length; j++)
                {
                    ...
                    if (parameters[j].ParameterType.IsGenericParameter
                        && this.genericTypes.Length > parameters[j].ParameterType.GenericParameterPosition
                        && this.genericTypes[parameters[j].ParameterType.GenericParameterPosition].IsAssignableFrom(types[j]))
                    {
                        count += 1;
                    }
