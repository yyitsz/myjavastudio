...
import org.yy.common.web.auth.impl.PropertiesAuthConfig;
	...
	private void processRequest(HttpServletRequest req,HttpServletResponse res,FilterChain chain) throws IOException, ServletException{
		...
		if (auth != null && auth.urlAccessible(servletPath)) {
			...
			if (auth == null || auth.getUser() == null) {
				...
				res.sendRedirect(req
						.getContextPath()
						+ AuthManager.getAuthConfig().getLoginPage()+"?lasturl="+URLEncoder.encode(lasturl,"ISO-8859-1"));
			} else {
				res.sendRedirect(req
						.getContextPath()
						+ AuthManager.getAuthConfig().getDeniedPage());
			}
		}
	}
