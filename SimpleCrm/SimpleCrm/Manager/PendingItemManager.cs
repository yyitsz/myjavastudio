...
namespace SimpleCrm.Manager
{
    public class PendingItemManager : BaseRepo<PendingItem, long?>
    {
        ...
        {
            return Connection.Query<PendingItemDto>(@"select  cust.CustomerName CustomerName, cust.CustomerId RefId, cust.BirthdayThisYear ActionDate, cust.Birthday, pi.PendingItemId PendingItemId, 'Birthday' Category,pi.CreateTime CreateTime
...
from (select c.*, date(c.Birthday, '+' || (((strftime('%Y', 'now') -  strftime('%Y', c.Birthday)) * 12 + (strftime('%m', 'now') -  strftime('%m', c.Birthday)) + 11) /12) || ' years') BirthdayThisYear
     from Customer c 
     where c.Birthday is not null) cust
		LEFT JOIN PendingItem pi on cust.CustomerId = pi.RefId and pi.Category = 'Birthday' and cust.BirthdayThisYear = date(pi.ActionDate)
	where cust.BirthdayThisYear >= date(@FromDate) and cust.BirthdayThisYear <= date(@ToDate)
...
from (select ip.*, date(ip.EffectiveDate, '+' || (((strftime('%Y', 'now') -  strftime('%Y', ip.EffectiveDate)) * 12 + (strftime('%m', 'now') -  strftime('%m', ip.EffectiveDate)) + 11) /12) || ' years') RenewalDate
    ...
    INNER JOIN Customer cust on ipc.CustomerId = cust.CustomerId
		LEFT JOIN PendingItem pi on policy.InsurancePolicyId = pi.RefId and pi.Category = 'RenewalPremium' and policy.RenewalDate = date(pi.ActionDate)
	where policy.RenewalDate >= date(@FromDate) and policy.RenewalDate <= date(@ToDate)
